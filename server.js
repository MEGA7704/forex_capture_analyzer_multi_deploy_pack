...
function verifyToken(token, secret){ const parts = String(token || '').split('.'); if (parts.length !== 3) throw new Error('Token invalide'); const [h,p,sig] = parts; const body = `${h}.${p}`; const expected = crypto.createHmac('sha256', secret).update(body).digest('base64url'); if (sig !== expected) throw new Error('Signature invalide'); const payload = JSON.parse(Buffer.from(p, 'base64url').toString('utf8')); if (payload.exp && Date.now() > payload.exp * 1000) throw new Error('Session expirée'); return payload; }
function requireAdmin(req){ const auth = req.headers.authorization || ''; const token = auth.replace(/^Bearer\s+/i, '').trim(); if (!token || token !== ADMIN_TOKEN) throw new Error('Accès admin refusé'); }
function pickClientIp(req){ return req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket?.remoteAddress || ''; }

app.get('/health', async (_req, res) => {
  try {
    const licenses = await readLicenses();
    res.json({ ok:true, status:'up', licenses: licenses.length, dataDir: BASE_DATA_DIR });
  } catch (err) {
...
app.post('/api/license/report', async (req, res) => {
  try{
    const token = String(req.body?.token || '').trim();
    const type = String(req.body?.type || 'analysis_run').trim();
