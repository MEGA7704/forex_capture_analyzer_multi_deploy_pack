...
const LICENSE_SECRET = String(process.env.LICENSE_SECRET || 'CHANGE_ME_LICENSE_SECRET').trim();
const ADMIN_TOKEN = String(process.env.ADMIN_TOKEN || 'CHANGE_ME_ADMIN_TOKEN').trim();
...
function normalizeLicenseShape(license) {
  ...
  license.price_usdt = toFiniteNumber(license.price_usdt, 0);
...
function getLicenseAccessError(license) {
  ...
  if (license.mode === 'duration' && (license.status === 'expired' || isExpired(license))) {
    return 'Licence expirée';
  }
  if (license.mode === 'count' && (license.status === 'quota_reached' || isCountExhausted(license))) {
...
function safeIpHash(ip = '') {
  ...
  return 'ip_' + normalized.split('.').slice(0, 2).join('_');
}
...
function wrapLicensesPayload(items) {
  return {
    licenses: (items || []).map((item) => refreshComputedStatus(item))
  };
}
...
async function ensureFiles() {
  ...
  if (!(await fileExists(LICENSE_FILE))) {
    if (await fileExists(SEED_LICENSE_FILE)) {
      const seed = await fs.readFile(SEED_LICENSE_FILE, 'utf8');
      await fs.writeFile(LICENSE_FILE, seed, 'utf8');
    } else {
  ...
  if (!(await fileExists(EVENTS_FILE))) {
    if (await fileExists(SEED_EVENTS_FILE)) {
      const seedEvents = await fs.readFile(SEED_EVENTS_FILE, 'utf8');
      await fs.writeFile(EVENTS_FILE, seedEvents, 'utf8');
    } else {
...
async function githubRequest(url, options = {}) {
  if (typeof fetch !== 'function') {
    throw new Error('Fetch non disponible. Utilise Node.js 18+ sur Render.');
  }
...
async function githubPutJsonFile(remotePath, jsonData, message) {
  ...
  const body = {
    ...
    committer: {
      name: 'Forex Capture Analyzer Bot',
      email: 'bot@local.invalid'
    }
  };
  ...
  return {
    ok: true,
    sha: githubCache.remoteShaByPath[remotePath]
  };
}
...
async function syncDownFromGitHub(force = false) {
  ...
  if (remoteLicenses.ok) {
    await fs.writeFile(LICENSE_FILE, JSON.stringify(remoteLicenses.json, null, 2), 'utf8');
  ...
  if (remoteEvents.ok) {
    await fs.writeFile(EVENTS_FILE, JSON.stringify(remoteEvents.json, null, 2), 'utf8');
...
async function persistLicenses(items, reason = 'Update licenses') {
  ...
  await fs.writeFile(LICENSE_FILE, JSON.stringify(payload, null, 2), 'utf8');
...
async function readEvents({ forcePull = false } = {}) {
  await ensureFiles();
  await syncDownFromGitHub(forcePull);
  const items = await readJsonSafe(EVENTS_FILE, []);
...
app.post('/api/license/portal', async (req, res) => {
  try {
    ...
    const firstActivation =
      !license.activated_at || String(license.status || '').toLowerCase() === 'unused';
...
app.post('/api/license/validate', async (req, res) => {
  try {
    ...
    if (!claimedDevice) return res.status(400).json({ ok: false, error: 'Appareil requis' });
...
app.post('/api/license/report', async (req, res) => {
  try {
    ...
    if (type === 'error_report') {
      license.error_reports = Number(license.error_reports || 0) + 1;
    }

    if (type === 'sos_report') {
      license.sos_reports = Number(license.sos_reports || 0) + 1;
    }
...
app.post('/api/license/block', async (req, res) => {
  try {
    ...
    let license = await getLicense(key);
    if (!license) {
      return res.status(404).json({ ok: false, error: 'Licence introuvable' });
    }
    ...
    if (action === 'block') {
      ...
      const anchor =
        license.mode === 'duration' && isExpired(license)
...
app.post('/api/admin/licenses/generate-bulk', async (req, res) => {
  try {
    ...
    await writeLicenses(
      [...current, ...generated],
      `Bulk create ${generated.length} licenses for ${plan.plan_id}`
    );
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(
      `GitHub sync: ${
        GITHUB_SYNC_ENABLED
          ? `enabled (${GITHUB_REPO}@${GITHUB_BRANCH})`
          : 'disabled'
      }`
    );
  } catch (err) {
